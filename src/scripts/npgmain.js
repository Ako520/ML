...
//License: BSD
//This export function does all the boring canvas stuff. To use it, just create export functions:
...
let g_iFPS = 10; // 帧数，每秒刷新次数
let g_strFinalLineStrokeStype = 'rgb(0,0,255)'; // 最终生成的线性图形颜色
let g_iNumOfData = 0; // 默认数据个数
...
var g_iDataPointSize = 2;
var avloss = 0.0;


export function drawBubble(x, y, w, h, radius)
{
  var r = x + w;
  var b = y + h;
  ctx.beginPath();
...
export function randn(mean, variance) {
  var V1, V2, S;
  do {
    var U1 = Math.random();
    var U2 = Math.random();
...
export function eventKeyUp(e) {
  var keycode = ('which' in e) ? e.which : e.keyCode;
  keyUp(keycode);
}

export function eventKeyDown(e) {
  var keycode = ('which' in e) ? e.which : e.keyCode;
  keyDown(keycode);
}
...
export function updateLix(newlix) {
  $("#button"+lix).css('background-color', ''); // erase highlight
  lix = newlix;
  $("#button"+lix).css('background-color', '#FFA');
}

export function regen_data() {
...
export function mouseClick(x, y, shiftPressed){
